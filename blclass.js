...
{ 
    ...
    this.blMV = function(id,html,x,y,w,h,bkClr){
		var md = this.blDiv(document.body, id, html,bkClr);  
		if(!md.run){
			md.run = true; 
			var style ="position: absolute;";
			style += "z-index: 9;"; 
			...
			md.style.width = w+"px";			
			md.style.height = h+"px";	
			md.style.backgroundColor = bkClr;
	...
	this.blsXY = function(x,y){ return "["+x+","+y+"]";}
...
function C1Script(_id,_x,_y){
	...
	var rc = new CRectBtn(X,Y,w,h,"green","yellow");
	var vTest = null;
	var _this = this;
	var r1 = new CBtn(X+W-w,Y,w,h,"blue",function(){ 
		var d = new Date();
		var n = d.toLocaleTimeString();
		if(!vTest){			
			var p = _getXY();		
			vTest = blo0.blMV("id_vTest"+id,"",p.x,p.y,10,10,"red"); 
			vTest.onmousemove = function(){		
				var c = _getXY();		
				if(rc.getStatus()){
					clickTest = blo0.blsXY(c.x,c.y) ;
					_this.setX(c.x);
					_this.setY(c.y);
				}
			}
	...
	this.draw1Script = function(cvs){   
		...
		if(r.getStatus()){
			blo0.blRect(cvs,X,Y,W,H,C);
			blo0.blText(cvs,blo0.blsXY(X,Y) + " sid=" + id + " z="+z ,X+W/4,Y+H/5,12,"green"); 
...
function CScriptMng(){
	...
	this.drawList_Z_0_N = function(cvs,_x,_y,_c){ 
		blo0.blRect(cvs,_x,_y,11,21,_c);
		blo0.blText(cvs,"Z_0_N: ",_x,_y,_c);
		var n = 0;
		var l = ls.length;
		blo0.blText(cvs,n,_x+n*15+20,_y+11,_c);	
		for(var z = 0; z<=l;z++){
			blo0.blText(cvs,n,_x+n*15+20,_y+22,"yellow");	
			for(i in ls){
				if(z==ls[i].getZ()){
	...
	this.drawMng = function(cvs){ 
		...
		if(r.getStatus()){
			...
			blo0.blText(cvs,saj,X+W/15,Y+H/1.8,12,"red");
			...
			this.drawList_Z_0_N(cvs,X,Y+H/1.5,"blue"); 
		}
	} 
